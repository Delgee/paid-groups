...
export class MessageTemplateService {
  ...
  /**
   * Get template from project or use default
   */
  getTemplate(
    projectTemplates: MessageTemplates | Record<string, any>,
    templateName: keyof MessageTemplates,
  ): string {
    return projectTemplates?.[templateName] || DEFAULT_TEMPLATES[templateName] || '';
  }
  ...
  buildWelcomeMessage(
    ...
    const template = this.getTemplate(projectTemplates, 'welcome_message');
  ...
  buildPaymentSuccessMessage(
    ...
    const template = this.getTemplate(projectTemplates, 'payment_success');
    return this.processTemplate(template, variables);
  }
  ...
  buildPaymentFailedMessage(
    ...
    const template = this.getTemplate(projectTemplates, 'payment_failed');
    return this.processTemplate(template, variables);
  }
  ...
  buildExpirationWarningMessage(
    ...
    const template = this.getTemplate(projectTemplates, templateName);
    return this.processTemplate(template, variables);
  }
  ...
  buildMembershipExpiredMessage(
    ...
    const template = this.getTemplate(projectTemplates, 'membership_expired');
    return this.processTemplate(template, variables);
  }
